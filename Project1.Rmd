...
avg_ret_1yr_m  <- mean(mkt_ret_rolling$ret_1yr, na.rm = TRUE)
avg_ret_10yr_m <- mean(mkt_ret_rolling$ret_10yr, na.rm = TRUE)
avg_ret_20yr_m <- mean(mkt_ret_rolling$ret_20yr, na.rm = TRUE)
...
avg_ret_1yr_y  <- mean(mkt_ret_rolling_y$ret_1yr, na.rm = TRUE)
avg_ret_10yr_y <- mean(mkt_ret_rolling_y$ret_10yr, na.rm = TRUE)
avg_ret_20yr_y <- mean(mkt_ret_rolling_y$ret_20yr, na.rm = TRUE)
...
results %>%
  ...
  tab_header(title = "Average Returns of the Market Portfolio")
```
...
avg_ret_10yr_y_ann <- mean(mkt_ret_rolling_y_ann$ret_10yr_ann, na.rm = TRUE)
avg_ret_20yr_y_ann <- mean(mkt_ret_rolling_y_ann$ret_20yr_ann, na.rm = TRUE)
...
results %>%
  ...
  tab_header(title = "Average Annualized Returns of the Market Portfolio")
...
bootstrap_se <- function(returns, num_samples = 10000) {
  boot_means <- replicate(num_samples, mean(sample(returns, replace = TRUE)))  
...
result_se <- data.frame(
  ...
  Average_1_year_Return = round(se_1yr,4) ,
  Average_10_year_Return = round(se_10yr,4),
  Average_20_year_Return = round(se_20yr,4)
)
...
result_se %>%
  ...
  tab_header(title = "Bootstrap Standard Error of Average Annualized Returns")
...
ggplot(mkt_ret_monthly_long, aes(x = date, y = Compound_Return, color = Security)) +
  ...
  labs(
    title = "Compounded Monthly Returns Market Portfolio and Riskfree Rate",
    x = "Date",
    y = "Compounded Return",
    color = "Security"
  ) +
...
for (j in 1:2){
  for (i in 1:3) {
    ...
    avg_an_ret_[i] <- (1 + mean(ret_ALNY[[i]])) ^ time_2[i] - 1
    
    # Compound Annual Return
    comp_returns <- cumprod(1 + ret_ALNY[[i]])
    # Annualize the return by adjusting for the number of periods
    comp_an_ret_[i] <- (comp_returns[length(comp_returns)]) ^ (time_2[i] / length(comp_returns)) - 1
  }
...
colnames(summary_ret[[j]]) <- c("Period", "Arithmetic Average Annual Return", "Compound Annual Return")
print(summary_ret[[j]] %>%
  gt() %>%
  tab_style(
    style = cell_borders(sides = "right",
  ...
  tab_header(title = paste0("Arithmic Average Annualized Returns vs Monthly Returns of ",type[j]))
)
}
...
tab_header(title = "Estimated the Return Volatility for Alnylam and Russel 3000 Index")
...
returns_weekly$rolling_sd_ALNY <- rollapply(returns_weekly$ALNY, width = 140, FUN = sd, align = "right", fill = NA)
returns_weekly$rolling_sd_RUS <- rollapply(returns_weekly$Russel3000, width = 140, FUN = sd, align = "right", fill = NA)
